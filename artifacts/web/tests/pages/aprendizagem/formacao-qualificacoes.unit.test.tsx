...
describe("FormacaoQualificacoes", () => {
  ...
  it("mostra o chip de evidência nas linhas com atestado manual (anexo ou texto), casando por id", () => {
    // Competências manuais casadas por id via `manualCompetencyId` do requisito:
    //  11 (Atende Manual): só anexo -> chip "1 anexo"
    //  12 (Gap com Atestado): só texto -> chip com o texto
    //  13 (Atende Treinamento com Atestado): texto tem precedência sobre contagem
    const competencies = [
      {
        id: 11,
        name: "Atende Manual",
        ...
        acquiredLevel: 2,
        evidence: null,
        attachments: [{ objectPath: "a.pdf", name: "a.pdf" }],
      },
      {
        id: 12,
        name: "Gap com Atestado",
        type: "habilidade",
        requiredLevel: 4,
        acquiredLevel: 2,
        evidence: "Certificado X",
        attachments: [],
      },
      {
        id: 13,
        name: "Atende Treinamento com Atestado",
        type: "atitude",
        requiredLevel: 1,
        acquiredLevel: 1,
        evidence: "Cert Y",
        attachments: [{ objectPath: "y.pdf", name: "y.pdf" }],
      },
    ] as never;
    ...
    expect(screen.getByText("Certificado X")).toBeInTheDocument(); // id 12: texto
    expect(screen.getByText("1 anexo")).toBeInTheDocument(); // id 11: só anexo
    expect(screen.getByText("Cert Y")).toBeInTheDocument(); // id 13: texto > contagem
  });

  it("linha com manualCompetencyId mas sem prova (nem anexo nem texto) não mostra chip", () => {
    const competencies = [
      {
        id: 11,
        name: "Atende Manual",
    ...
    // Sem anexo e sem texto real -> nenhum chip de anexo/evidência.

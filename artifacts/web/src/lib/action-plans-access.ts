export type ActionPlanUserRole = "platform_admin" | "org_admin" | "operator" | "analyst" | "manager";

export interface ActionPlanRequesterScope {
  role: ActionPlanUserRole;
  userId: number;
...
function isAdmin(role: ActionPlanUserRole): boolean {
  return role === "org_admin" || role === "platform_admin";
}

/**
 * Matriz única de VISIBILIDADE do hub de Ações.
 * Espelha `artifacts/api-server/src/services/action-plans/access.ts` — manter em sync.
 * Só governa quem VÊ; a escrita é barrada à parte.
 */

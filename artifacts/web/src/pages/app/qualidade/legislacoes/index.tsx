...
import {
  ...
  useImportLegislations,
  getListLegislationsQueryKey,
  useListUnits,
  getListUnitsQueryKey,
  useGetUnitComplianceTags,
  getGetUnitComplianceTagsQueryKey,
  type CreateLegislationBody,
  type ListLegislationsLevel,
...
function parseXlsxRows(
  ...
  for (const row of rows) {
    ...
    const freqDays = mapped.reviewFrequencyDays
      ? parseInt(String(mapped.reviewFrequencyDays), 10)
      : undefined;
    ...
    results.push({
      ...
      municipality: mapped.municipality
        ? String(mapped.municipality).trim()
        : undefined,
      ...
      applicability: mapped.applicability
        ? String(mapped.applicability).trim().toLowerCase()
        : undefined,
      ...
      observations: mapped.observations
        ? String(mapped.observations).trim()
        : undefined,
      generalObservations: mapped.generalObservations
        ? String(mapped.generalObservations).trim()
        : undefined,
      tags: parsedTags,
    });
  }
...
function analyzeImport(
  ...
  for (const leg of existing) {
    if (leg.tipoNorma && leg.number) {
      existingKeys.add(
        `${leg.tipoNorma.trim().toLowerCase()}::${leg.number.trim().toLowerCase()}`,
      );
    }
  }
...
export default function LegislacoesPage() {
  ...
  const { data: legislations, isLoading } = useListLegislations(
    ...
    {
      search,
      level: (levelFilter || undefined) as ListLegislationsLevel | undefined,
      unitId: unitIdNum,
    },
    {
      query: {
        queryKey: [
          ...getListLegislationsQueryKey(orgId!),
          search,
          levelFilter,
          unitFilter,
        ],
  ...
  const unitHasNoTags =
    !!unitIdNum &&
    unitComplianceTags !== undefined &&
    unitComplianceTags.length === 0;
  ...
  const executeBulkDelete = async () => {
    ...
    try {
      for (const id of selectedIds) {
        try {
          await deleteLegMut.mutateAsync({ orgId: orgId!, legId: id });
        } catch {}
      }
  ...
  const [importResult, setImportResult] = useState<ImportResultType | null>(
    null,
  );
  ...
  const [conflictStrategy, setConflictStrategy] = useState<"skip" | "update">(
    "skip",
  );
    ...
    reader.onload = async (ev) => {
      ...
      if (mapped.length > 0) {
        const result = await importMut.mutateAsync({
          orgId,
          data: { legislations: mapped, conflictStrategy },
        });
  ...
  const onBatchAutoTag = async () => {
    ...
    try {
      ...
      const res = await fetch(
        resolveApiUrl(
          `/api/organizations/${orgId}/legislations/auto-tag/batch`,
        ),
        {
          method: "POST",
          headers: {
    ...
    return canWriteLegislations ? (
      <>
        <HeaderActionButton
          ...
          label={isAutoTagging ? "Classificando..." : "Classificar Tags"}
          icon={
            isAutoTagging ? (
  ...
  return (
    <>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="text-muted-foreground mb-2">
                      Esta unidade ainda não possui tags de conformidade.
                    </div>
                    <div className="text-sm text-muted-foreground">
                      Preencha o questionário da unidade para gerar as tags e
                      filtrar as legislações aplicáveis.
                    </div>
                  </td>
                </tr>
              ) : legislations?.length === 0 ? (
      ...
      <Dialog
        open={isAutoTagging || autoTagComplete}
        onOpenChange={(open) => {
          if (!open) resetAutoTag();
        }}
        title="Auto-classificação de Tags"
      >
          ...
          {autoTagComplete && autoTagProgress ? (
            <div className="py-6 text-center space-y-1">
              ...
              <p className="text-[13px] text-muted-foreground">
                {autoTagProgress.tagged} legislaç
                {autoTagProgress.tagged === 1
                  ? "ão classificada"
                  : "ões classificadas"}
                {autoTagProgress.errors > 0 && (
                  <>
                    , {autoTagProgress.errors} erro
                    {autoTagProgress.errors > 1 ? "s" : ""}
                  </>
                )}
              </p>
            </div>
            ...
            <div className="py-4 space-y-4">
              <div className="text-center">
                ...
                <p className="text-base font-semibold">
                  Classificando legislações com IA...
                </p>
              ...
              <div className="w-full bg-secondary rounded-full h-2.5 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${autoTagProgress.total > 0 ? (autoTagProgress.current / autoTagProgress.total) * 100 : 0}%`,
                  }}
                />
              </div>
            </div>
      ...
      <Dialog
        ...
        title="Cadastrar Legislação"
        description="Adicione uma nova legislação ao sistema."
        size="lg"
      >
        <form onSubmit={form.handleSubmit(onCreateSubmit)}>
          <div className="grid grid-cols-2 gap-x-8 gap-y-5">
            ...
            <div>
              ...
              <Input
                {...form.register("applicableArticles")}
                placeholder="Ex: Art. 2°, Art. 4°, Art. 9°"
              />
            </div>
            <div className="col-span-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit" size="sm" isLoading={createMut.isPending}>
              Salvar
            </Button>
          </DialogFooter>
        </form>
        ...
        <div className="mt-4 space-y-5 text-sm">
          {importResult ? (
            <div className="py-4 space-y-4">
              ...
              <div className="text-center">
                <Button onClick={resetImport} className="mt-1">
                  Fechar
                </Button>
              </div>
            </div>
                ...
                {importPreview && importPreview.existingCount > 0 && (
                  <div className="space-y-2">
                    <p className="text-[13px] font-medium text-foreground">
                      {importPreview.existingCount} legislações já existem
                      (identificadas por Tipo + Número). O que fazer com elas?
                    </p>
                    ...
                    {importPreview.existingItems.length > 0 && (
                      <details className="text-[12px] text-muted-foreground">
                        <summary className="cursor-pointer hover:text-foreground">
                          Ver exemplos de legislações já cadastradas (
                          {importPreview.existingCount > 10
                            ? `mostrando 10 de ${importPreview.existingCount}`
                            : importPreview.existingCount}
                          )
                        </summary>
                        <ul className="mt-1 space-y-0.5 pl-4 list-disc">
                          {importPreview.existingItems.map((item, i) => (
                            <li key={i}>
                              {item.tipoNorma} {item.number}
                            </li>
                          ))}
                        </ul>
                      </details>
      ...
      >
        <p className="text-sm text-muted-foreground mt-2">
          Tem certeza que deseja excluir {selectedIds.size} legislaç
          {selectedIds.size > 1 ? "ões" : "ão"}?
        </p>
        <DialogFooter>
          <Button
            type="button"
            ...
            <TooltipProvider delayDuration={200}>
              <div className="flex items-center gap-1 rounded-xl border border-border/60 bg-card/90 px-3 py-2 shadow-lg backdrop-blur-xl">
                <span className="px-2 text-[13px] font-medium text-foreground">
                  {selectedIds.size} selecionad
                  {selectedIds.size > 1 ? "as" : "a"}
                </span>

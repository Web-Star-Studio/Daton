...
const SuggestDocumentNormativeRequirementsBodySchema = z.object({
...
async function validateOrgEmployees(
  employeeIds: number[],
  orgId: number,
): Promise<boolean> {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        eq(unitsTable.organizationId, orgId),
      ),
    );
  return rows.length === unitIds.length;
}

async function validateOrgDocuments(
  docIds: number[],
  orgId: number,
): Promise<boolean> {
...
async function getEmployeeLinkedUserIds(
  employeeIds: number[],
  orgId: number,
): Promise<number[]> {
...
async function getDocumentParticipantUserIds(
  docId: number,
  orgId: number,
): Promise<number[]> {
  const [doc, elaborators, criticalReviewers, approvers, recipients] =
    await Promise.all([
      db
...
async function getDocumentReviewStakeholderUserIds(
  docId: number,
  orgId: number,
): Promise<number[]> {
...
async function getDocumentDraftStakeholderUserIds(
  docId: number,
  orgId: number,
): Promise<number[]> {
...
async function getDocumentDetail(docId: number, orgId: number) {
  ...
  const elaboratorRows = await db
    ...
    .innerJoin(
      employeesTable,
      eq(documentElaboratorsTable.employeeId, employeesTable.id),
    )
  ...
  const maxCriticalAnalysisCycleResult = await db
    ...
    .from(documentCriticalAnalysisTable)
    .where(eq(documentCriticalAnalysisTable.documentId, docId));
  const currentCriticalAnalysisCycle =
    maxCriticalAnalysisCycleResult[0]?.maxCycle ?? 0;
  ...
  const criticalReviewerRows =
    currentCriticalAnalysisCycle > 0
      ? await db
          ...
          .innerJoin(
            usersTable,
            eq(documentCriticalAnalysisTable.userId, usersTable.id),
          )
          .where(
            and(
              eq(documentCriticalAnalysisTable.documentId, docId),
              eq(
                documentCriticalAnalysisTable.analysisCycle,
                currentCriticalAnalysisCycle,
              ),
            ),
          )
      : await db
          ...
          .innerJoin(
            usersTable,
            eq(documentCriticalReviewersTable.userId, usersTable.id),
          )
          .where(eq(documentCriticalReviewersTable.documentId, docId));

  const maxCycleResult = await db
    .select({
      maxCycle: sql<number>`COALESCE(MAX(${documentApproversTable.approvalCycle}), 1)`,
    })
    .from(documentApproversTable)
    .where(eq(documentApproversTable.documentId, docId));
  ...
  const refRows = await db
    ...
    .innerJoin(
      documentsTable,
      eq(documentReferencesTable.referencedDocumentId, documentsTable.id),
    )
  ...
  const attachmentRows = await db
    ...
    .leftJoin(
      usersTable,
      eq(documentAttachmentsTable.uploadedById, usersTable.id),
    )
  ...
  return {
    ...
    normativeRequirements: normalizeNormativeRequirements(
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    if (query.data.unitId) {
      const unitDocIds = await db
        .select({ documentId: documentUnitsTable.documentId })
        .from(documentUnitsTable)
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const createBody = body.data as typeof body.data & {
    ...
    if (
      body.data.unitIds?.length &&
      !(await validateOrgUnits(body.data.unitIds, orgId))
    ) {
      res.status(400).json({
        error:
          "Uma ou mais filiais selecionadas não pertencem a esta organização",
      });
      return;
    }
    if (
      body.data.referenceIds?.length &&
      !(await validateOrgDocuments(body.data.referenceIds, orgId))
    ) {
      res.status(400).json({
        error:
          "Um ou mais documentos referenciados não pertencem a esta organização",
      });
      return;
    }
    if (
...
router.post(
  "/organizations/:orgId/documents/normative-requirements/suggestions",
  requireAuth,
  requireModuleAccess("documents"),
  requireWriteAccess(),
  async (req, res): Promise<void> => {
    ...
    const body = SuggestDocumentNormativeRequirementsBodySchema.safeParse(
      req.body,
    );
    if (!body.success) {
      res.status(400).json({ error: body.error.message });
    ...
    const referenceIds = [...new Set(body.data.referenceIds ?? [])];
    if (
      referenceIds.length > 0 &&
      !(await validateOrgDocuments(referenceIds, params.data.orgId))
    ) {
      res.status(400).json({
        error:
          "Um ou mais documentos referenciados não pertencem a esta organização",
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const policyCheck = await assertPolicyDocument(
      params.data.docId,
      params.data.orgId,
    );
    if ("error" in policyCheck) {
      ...
      res
        .status(400)
        .json({ error: "Apenas políticas aceitam planos de comunicação SGQ" });
      return;
    }
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const body = DocumentCommunicationPlanBodySchema.partial().safeParse(
      req.body,
    );
    if (!body.success) {
      res.status(400).json({ error: body.error.message });
    ...
    if (body.data.audience !== undefined)
      updateData.audience = body.data.audience;
    if (body.data.periodicity !== undefined)
      updateData.periodicity = body.data.periodicity;
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    if (existing.status !== "draft" && existing.status !== "rejected") {
      res.status(400).json({
        error: "Apenas documentos em rascunho ou rejeitados podem ser editados",
      });
      return;
    }
    ...
    if (
      body.data.elaboratorIds?.length &&
      !(await validateOrgEmployees(body.data.elaboratorIds, orgId))
    ) {
      res.status(400).json({
        error: "Um ou mais elaboradores não pertencem a esta organização",
      });
    ...
    const normalizedNormativeRequirements =
      updateBody.normativeRequirements !== undefined
        ? normalizeNormativeRequirements(updateBody.normativeRequirements)
        : undefined;
...
router.delete(
  ...
  async (req, res): Promise<void> => {
    ...
    if (existing.status !== "draft" && existing.status !== "rejected") {
      res.status(400).json({
        error:
          "Apenas documentos em rascunho ou rejeitados podem ser excluídos",
      });
      return;
    }
...
router.get(
  "/organizations/:orgId/documents/:docId/versions",
  requireAuth,
  async (req, res): Promise<void> => {
    ...
    const rows = await db
      ...
      .leftJoin(
        usersTable,
        eq(documentVersionsTable.changedById, usersTable.id),
      )
...
router.delete(
  ...
  async (req, res): Promise<void> => {
    ...
    if (doc.status !== "draft" && doc.status !== "rejected") {
      res.status(400).json({
        error:
          "O histórico de versões só pode ser resetado em documentos em rascunho ou rejeitados",
      });
      return;
    }
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (doc.status !== "draft" && doc.status !== "rejected") {
      res.status(400).json({
        error:
          "Anexos só podem ser adicionados em documentos em rascunho ou rejeitados",
      });
      return;
    }

    const userId = req.auth!.userId;
    const criticalReviewerIds = await getDocumentCriticalReviewerUserIds(
      doc.id,
    );
...
router.delete(
  "/organizations/:orgId/documents/:docId/attachments/:attachId",
  requireAuth,
  requireModuleAccess("documents"),
  requireWriteAccess(),
  async (req, res): Promise<void> => {
    ...
    if (doc.status !== "draft" && doc.status !== "rejected") {
      res.status(400).json({
        error:
          "Anexos só podem ser removidos em documentos em rascunho ou rejeitados",
      });
      return;
    }
    ...
    try {
      ...
      if (
        ...
        error.message === "DOCUMENT_ATTACHMENT_NOT_FOUND"
      ) {
...
router.get(
  "/organizations/:orgId/documents/:docId/attachments/:attachId/file",
  requireAuth,
  async (req, res): Promise<void> => {
    const params = DeleteDocumentAttachmentParams.safeParse(req.params);
    if (!params.success) {
    ...
    const [attachment] = await db
      ...
      .innerJoin(
        documentsTable,
        eq(documentAttachmentsTable.documentId, documentsTable.id),
      )
      .where(
        and(
          eq(documentAttachmentsTable.id, params.data.attachId),
...
router.post(
  "/organizations/:orgId/documents/:docId/critical-analysis/complete",
  requireAuth,
  requireModuleAccess("documents"),
  async (req, res): Promise<void> => {
    const params = CompleteDocumentCriticalAnalysisParamsSchema.safeParse(
      req.params,
    );
    if (!params.success) {
      res.status(400).json({ error: params.error.message });
    ...
    if (doc.status !== "draft") {
      res.status(400).json({
        error:
          "A análise crítica só pode ser concluída para documentos em análise crítica",
      });
      return;
    }

    const currentCriticalAnalysisCycle =
      await getCurrentCriticalAnalysisCycle(docId);
    if (currentCriticalAnalysisCycle === 0) {
      res
        .status(400)
        .json({ error: "O documento não possui uma análise crítica ativa" });
      return;
    }
    ...
    if (!criticalReview) {
      res.status(403).json({
        error:
          "Você não é um responsável ativo pela análise crítica deste documento",
      });
      return;
    }
    if (criticalReview.status === "completed") {
      res
        .status(400)
        .json({ error: "Sua análise crítica já foi concluída neste ciclo" });
      return;
    }
...
router.post(
  "/organizations/:orgId/documents/:docId/submit",
  requireAuth,
  requireModuleAccess("documents"),
  requireWriteAccess(),
  async (req, res): Promise<void> => {
    ...
    if (doc.status !== "draft" && doc.status !== "rejected") {
      res
        .status(400)
        .json({ error: "Documento não pode ser submetido neste estado" });
      return;
    }
    ...
    if (req.auth!.role !== "org_admin" && req.auth!.role !== "operator") {
      res.status(403).json({
        error:
          "Apenas administradores e operadores podem enviar o documento para revisão",
      });
      return;
    }
    ...
    if (criticalReviewerIds.length === 0) {
      res.status(400).json({
        error:
          "O documento deve ter pelo menos um responsável pela análise crítica",
      });
      return;
    }
    ...
    if (currentCriticalAnalysisCycle === 0) {
      res.status(400).json({
        error: "A análise crítica do documento ainda não foi iniciada",
      });
      return;
    }
    ...
    if (pendingCriticalReviewers.length > 0) {
      res.status(400).json({
        error:
          "Conclua a análise crítica antes de enviar o documento para revisão",
      });
      return;
    }
    ...
    const maxCycleResult = await db
      .select({
        maxCycle: sql<number>`COALESCE(MAX(${documentApproversTable.approvalCycle}), 0)`,
      })
      .from(documentApproversTable)
      .where(eq(documentApproversTable.documentId, docId));
    const newCycle = (maxCycleResult[0]?.maxCycle ?? 0) + 1;

    const distinctApprovers = await db
      .selectDistinct({ userId: documentApproversTable.userId })
      .from(documentApproversTable)
      .where(eq(documentApproversTable.documentId, docId));
    ...
    if (distinctApprovers.length === 0) {
      res.status(400).json({
        error:
          "O documento deve ter pelo menos um aprovador antes de ser enviado para revisão",
      });
      return;
    }
...
router.post(
  "/organizations/:orgId/documents/:docId/approve",
  requireAuth,
  requireModuleAccess("documents"),
  requireWriteAccess(),
  async (req, res): Promise<void> => {
    ...
    if (!approver) {
      res
        .status(403)
        .json({ error: "Você não é um aprovador pendente deste documento" });
      return;
    }
...
router.post(
  "/organizations/:orgId/documents/:docId/distribute",
  requireAuth,
  requireModuleAccess("documents"),
  requireWriteAccess(),
  async (req, res): Promise<void> => {
    ...
    if (doc.status !== "approved") {
      res.status(400).json({
        error: "Documento precisa estar aprovado para ser distribuído",
      });
      return;
    }
...
router.post(
  "/organizations/:orgId/documents/:docId/acknowledge",
  requireAuth,
  async (req, res): Promise<void> => {
    ...
    if (doc.status !== "distributed") {
      res.status(400).json({ error: "Documento não está distribuído" });
      return;
    }
    ...
    const [recipient] = await db
      ...
      .where(
        and(
          eq(documentRecipientsTable.documentId, docId),
          eq(documentRecipientsTable.userId, userId),
        ),
      );
    if (!recipient) {
      res
        .status(403)
        .json({ error: "Você não é um destinatário deste documento" });
      return;
    }
    ...
    const acknowledgmentAudience = await getDocumentReviewStakeholderUserIds(
      docId,
      orgId,
    );
    await notifyUsers({
...
router.get(
  "/organizations/:orgId/user-options",
  requireAuth,
  requireModuleAccess("documents"),
  async (req, res): Promise<void> => {
    const orgId = parseInt(
      Array.isArray(req.params.orgId)
        ? (req.params.orgId[0] ?? "")
        : (req.params.orgId ?? ""),
      10,
    );
    if (isNaN(orgId)) {
      res.status(400).json({ error: "orgId inválido" });
      return;
    }

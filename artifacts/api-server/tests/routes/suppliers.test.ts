...
describe("suppliers routes", () => {
  ...
  it("creates a supplier and rejects category/type/unit references from another organization", async () => {
    ...
    const foreignContext = await createTestContext({ seed: "suppliers-foreign" });
    ...
    const foreignCategory = await createSupplierCategory(foreignContext, "Externo");
  ...
  it("requires an apt document review before qualification and updates supplier status", async () => {
    const context = await createTestContext({ seed: "suppliers-qualification" });
    contexts.push(context);
    ...
    expect(blockedQualification.body.error).toContain("avaliação documental apta");
  ...
  it("allows operators to create receipt checks, validates refs and records failures", async () => {
    ...
    const foreignContext = await createTestContext({ seed: "suppliers-receipts-foreign" });
    contexts.push(context, foreignContext);
    ...
    const manualFailure = await request(app)
      .post(`/api/organizations/${context.organizationId}/suppliers/${supplier.id}/failures`)
      .set(authHeader(context))
      .send({

...
The Daton platform is built as a pnpm workspace monorepo utilizing TypeScript. The frontend is developed with React, Vite, Tailwind CSS for styling, wouter for routing, and React Query for data fetching. The backend API is powered by Express 5, interacting with a PostgreSQL database managed through Drizzle ORM. Authentication is handled via Replit Auth (OpenID Connect with PKCE), using httpOnly session cookies stored in a PostgreSQL `sessions` table. Users are linked via `replitId` on the existing integer-ID `users` table. The `authMiddleware` loads the session on every request and sets `req.auth` (userId, organizationId) for downstream route handlers. The `@workspace/replit-auth-web` package provides a `useAuth()` hook for the frontend. Data validation is enforced using Zod, integrated with Drizzle. API client code is generated from an OpenAPI specification using Orval, ensuring type safety and consistency.

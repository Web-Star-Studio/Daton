...
- **Modelo de fragmentos (à prova de conflito entre branches):** o diário vive num branch dedicado **`diario`** (orphan). Cada entrada é um **arquivo próprio** em `docs/diario/<AAAA-MM-DD>/<HHMMSS>-<branch>.md` — **nunca se edita o arquivo de outra sessão**, então múltiplas sessões em branches diferentes alimentam o mesmo dia sem conflito. Convenção completa em `docs/diario/README.md`.
- **Registrar:** ao concluir uma tarefa/feature/correção significativa, rodar `python3 scripts/diario-add.py --modulo <X> --titulo "<...>" --file <entrada.md>` (ou conteúdo via stdin). O script grava o fragmento no branch `diario` (worktree temporário + commit + push com rebase-retry), de qualquer branch de trabalho; cria o branch `diario` na primeira entrada. **NÃO** editar os `docs/diario/*.md` consolidados na mão.
...
- **Consolidado + PDF:** gerar o MD do dia (e o PDF) a partir dos fragmentos com `python3 scripts/diario-build.py --day <AAAA-MM-DD> [--pdf]`. O gerador de PDF reutilizável é `scripts/gen-diario-pdf.py` (markdown → PDF com reportlab). O consolidado é artefato gerado (não versionar).
- **Versão executiva (friendly):** quando o relatório for para superiores/não-técnicos, gerar uma versão em linguagem de **negócio** (sem código, caminhos de arquivo ou nomes de biblioteca), focada em **valor/resultado**. ATENÇÃO: friendly **NÃO** é incompleto — **toda entrega significativa deve aparecer**, traduzida para o impacto, **nunca omitida**. Em especial NÃO ofuscar: migrações de dados, lançamento/refatoração de módulos, correções em produção e integrações. Antes de finalizar, conferir o resumo contra a lista de atividades do diário técnico e garantir que cada item relevante está representado. O diário técnico detalhado permanece como registro paralelo (para o time/auditoria).
